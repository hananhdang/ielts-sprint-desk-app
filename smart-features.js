(function () {
  ...
  const FEED_CACHE_KEY = 'ielts-bbc-feed-cache-v1';
  ...
  function installBBC() {
    ...
    const weekend=[0,6].includes(new Date().getDay()),defaultFeed=weekend?'news':'six';
    $('#bbcRecommendation').textContent=weekend?'今日推荐：Global News':'今日推荐：6 Minute English';
  ...
  async function loadFeed(key){
    ...
    box.innerHTML=items.map((x,i)=>{const id=x.guid||x.audio,done=state.listenedEpisodes[id];return`<article class="episode"><div class="meta">${esc(formatDate(x.date))} · BBC 官方音频</div><h3>${esc(x.title)}</h3><p class="small muted">${esc(x.description)}</p><audio controls preload="none" src="${esc(x.audio)}"></audio><div class="smart-actions"><a class="btn smallbtn" href="${esc(x.link)}" target="_blank" rel="noopener noreferrer">节目原页/文字稿 ↗</a><button class="btn smallbtn ${done?'primary':''}" data-listened="${esc(id)}">${done?'✓ 已完成':'标记听完'}</button></div></article>`}).join('');
