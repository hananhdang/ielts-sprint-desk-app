(function () {
  ...
  const FEED_CACHE_KEY = 'ielts-listening-feed-cache-v5';
  ...
  function renderAudioText(item) {
    const transcript=String(item?.transcript||'').trim(),translation=String(item?.translationZh||'').trim();
    if(!transcript&&!translation)return '';
    const source=String(item?.transcriptSource||'').trim();
    return `<div class="audio-text-folds">${transcript?`<details><summary>英文原文${source?' · '+esc(source):''}</summary><div class="audio-text">${esc(transcript)}</div></details>`:''}${translation?`<details><summary>中文翻译</summary><div class="audio-text">${esc(translation)}</div></details>`:''}</div>`;
  }
